...
class SummarizedTranscript(BaseModel):
    ...
    next_action: Literal[
        "Escalation Needed", "Call back", "PTP Reminder call", "Query", "Dispute Raised"
    ] = Field(description="Next action according to the summary")
    next_action_date: str | None = Field(
        description="Date when will be the next action will be taken. Date is in ISO format in UTC Timezone only."
    )
